...
from skellysubs.skellysubs_core.translation_pipeline.models.load_language_models import load_language_configs, \
    LanguageConfig, get_language_configs
...
class TranslatedText(BaseModel):
    ...
    def initialize(cls, language_config: LanguageConfig):
        return cls(translated_text=NOT_TRANSLATED_YET_TEXT,
                   translated_language_name=language_config.language,
    ...
    def _split_chinese(self) -> list[str]:
        if "chinese" in self.translated_language_name.lower():
            raise ValueError(f"Cannot split Chinese text for language {self.translated_language_name}")
...
class TranslationsCollection(BaseModel):
  ...
  def has_translations(self) -> bool:
      return not any([translation.translated_text == NOT_TRANSLATED_YET_TEXT for translation in self.translations.values()])

  @classmethod
  def create(cls):
      # Load language configurations from YAML
      translations = {language: TranslatedText.initialize(language_config=config)
                      for language, config in get_language_configs().items()}
